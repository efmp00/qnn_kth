...
def mainSsv(qnnArch, numTrainingPairs, numberSupervisedPairs, lda, ep, trainingRounds,  delta, kind, alert=0):
    ...
    #ChatGPT-assisted customization
    for i in range(len(trainingData)):
        trainingData[i][1] = ((1 - delta) * trainingData[i][1] + delta * randomQubitState(qnnArch[-1])).unit()

    for i in range(len(trainingDataSv)):
        trainingDataSv[i][1] = ((1 - delta) * trainingDataSv[i][1] + delta * randomQubitState(qnnArch[-1])).unit()

    for i in range(len(trainingDataUsv)):
        trainingDataUsv[i][1] = ((1 - delta) * trainingDataUsv[i][1] + delta * randomQubitState(qnnArch[-1])).unit()
...
def fidelityMatrixRandomUnitary(qnnArch, numTrainingPairs):
    ...
    for i in range(0, numTrainingPairs):
        for j in range(0, i):
            ...
            fidMatrix[i][j] = abs(trainingData[i][1].dag() * trainingData[j][1])**2
            fidMatrix[j][i] = fidMatrix[i][j]  # ChatGPT-assisted customization
            # oder
